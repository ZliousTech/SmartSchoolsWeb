<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="QuestionsForms.aspx.cs" Inherits="SmartSchool.questionsweb.QuestionsForms" %>
        ...
        <main class="body-container">
            <div class="inner-row">
                ...
                <div class="head-body-col">
                    ...
                    <h2><%=Resources.Resource.QuestionsForms %></h2>
                </div>
...
<%--                    <h5>
                            ...
                            <table class="table" id="exportTable" style="width: 100%">
                                <thead>
                                    <tr>
                                        ...
                                        <th><%=Resources.Resource.Questions %></th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <asp:Repeater ID="ReptFormsQuestionsList" runat="server">
                                        <ItemTemplate>
                                            <tr scope="row">
                                                ...
                                                <td class="center">
                                                    <a href="EditForm.aspx?FGID=<%# Eval("FormGuidID") %>" class="btn btn-primary btn-xs">
                                                        <i class="fa fa-pencil"></i>
                                                    </a>
                                                    <a href="javascript:void(0)" onclick="DeleteForm('<%# Eval("FormGuidID")%>', '<%# Eval("IsApproved")%>')" class="btn btn-danger btn-xs">
                                                        <i class="fa fa-trash-o"></i>
                                                    </a>
                                                </td>
                                                <td class="center">
                                                    <a class="btn btn-success btn-xs" href="ViewQuestions.aspx?FGID=<%# Eval("FormGuidID") %>"><%=Resources.Resource.View%></a>
                                                    <a class="btn btn-warning btn-xs" href="AddQuestions.aspx?QBGID=<%# Eval("FormGuidID") %>"><%=Resources.Resource.Add%></a>
                                                </td>
                                            </tr>
                                        </ItemTemplate>
                                    </asp:Repeater>
        ...
        <script>
            ...
            function DeleteForm(FGID, IsApproved) {
                if (IsApproved === "True") {
                    swal("<%=Resources.Resource.CannotDeleteThisFormBecauseItIsApproved%>.", {
                        icon: "info",
                    });
                    return;
                }
                ...
                swal({
                    ...
                    text: "<%=Resources.Resource.TheQuestionsFormWillBeDeleted%>",
                    icon: "warning",
                    buttons: true,
                    dangerMode: true,
                }).then((willDelete) => {
                    if (willDelete) {
                        var result = DeleteQuestionsForm(FGID, Lang);
                        if (result === "success") {
                            setTimeout(function () {
                                window.location.reload();
                            }, 4000);
                            swal("<%=Resources.Resource.TheQuestionsFormHasBeenDeleted%>.", {
                                icon: "success",
                            });
                        } else {
                            swal(result, "", "warning");
                        ...
                        swal("<%=Resources.Resource.TheDeletionHasBeenCanceled%>.");
                    }
                });
            }

            function DeleteQuestionsForm(FGID, Lang) {
                var ResponsRes;
                $.ajax({
                    async: false,
                    type: "POST",
                    url: "../ssappws.asmx/DeleteQuestionsForm",
