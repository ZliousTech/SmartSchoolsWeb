<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ViewQuestions.aspx.cs" Inherits="SmartSchool.questionsweb.ViewQuestions" %>
...
<%--        <button onclick="topFunction()" id="scroll-btn" title="Go to top">
        ...
        <main class="body-container">
            <div class="inner-row">
                ...
                <div class="head-body-col">
                    ...
                    <h2><%=Resources.Resource.QuestionInformation %></h2>
                </div>
                                    ...
                                    <asp:Repeater ID="ReptQuestionsList" runat="server">
                                        <ItemTemplate>
                                            <tr scope="row">
                                                ...
                                                <td class="center">
                                                    <a href="EditQuestion.aspx?FGID=<%=hdfgid.Value %>&QGID=<%# Eval("QuestionGuidID") %>" class="btn btn-primary btn-xs">
                                                        <i class="fa fa-pencil"></i>
                                                    </a>
                                                    <a href="javascript:void(0)" onclick="DeleteQuestion('<%# Eval("QuestionGuidID")%>', '<%# Eval("IsApproved")%>')" class="btn btn-danger btn-xs">
        ...
        <script>
            ...
            function DeleteQuestion(FGID, IsApproved) {
                if (IsApproved === "True") {
                    swal("<%=Resources.Resource.CannotDeleteThisQuestionBecauseItIsApproved%>.", {
                        icon: "info",
                    });
                    return;
                }
                ...
                swal({
                    ...
                    text: "<%=Resources.Resource.TheQuestionsWillBeDeleted%>",
                    icon: "warning",
                    buttons: true,
                    dangerMode: true,
                }).then((willDelete) => {
                    if (willDelete) {
                        var result = DelQuestion(FGID, Lang);
                        if (result === "success") {
                            setTimeout(function () {
                                window.location.reload();
                            }, 4000);

                            swal("<%=Resources.Resource.TheQuestionHasBeenDeleted%>.", {
                                icon: "success",
                            });
                        } else {
                            swal(result, "", "warning");
            ...
            function DelQuestion(QGID, Lang) {
                var ResponsRes;
                $.ajax({
                    async: false,
                    type: "POST",
                    url: "../ssappws.asmx/DeleteQuestion",
                    data: "{'QGID':'" + QGID + "', 'Lang':'" + Lang + "'}",
